...
| 2025-12-4 | v1.0.3 | 1.在章节2.2 请求格式定义，增加audioBookOption进行多维度搜索；<br><br>2.在章节2.2 请求格式定义，增加batchGetPlayUrl根据trackid搜索内容地址<br><br>3.在章节2.3 响应格式定义，对playAudio增加total（总数），albumId（专辑Id，有声书类型返回）<br><br>4.增加示例2.2.3：根据声音id批量查询播放链接<br><br>5.增加示例2.3.3：返回根据声音id批量查询播放链接结果<br><br>6.在章节2.1.1 发送主题：connect/online，增加对servicePackageCode的说明：当用户购买了不同内容包服务，首次搜索该内容包的内容时需要发送connect/online，传入该内容包对应的servicespackagecode进行认证授权 | zw  | 无   |
...
| servicePackageCode | 服务套餐码 | string | 1   | 是   | 服务套餐码（当用户购买了不同内容包服务，首次搜索该内容包的内容时需要发送connect/online，传入该内容包对应的servicespackagecode进行认证授权） |
...
| |audioBookOption | intent为”listen_audiobook”时,允许通过此条件进行多维度搜索,具体字段见下面audioBookOption说明,传了此字段则会忽略content | Object | 3   | 否   |
|batchGetPlayUrl |trackIds | 声音Id(trackId)数组 | array | 3   | 是   |
...
| --- | --- | --- | --- | --- | --- |
| **字段** | **说明** | **格式** | **层级** | **是否必填** | **备注** |
| tag | 标签，多个使用英文逗号分隔 | string | 4   | 否   | &nbsp; |
| albumId | 专辑Id，返参的albumId或者全量同步接口拿到的Id | string | 4   | 否   | &nbsp; |
| albumName | 专辑名称，返参的albumName或者全量同步接口拿到的name，支持模糊搜索 | string | 4   | 否   | &nbsp; |
| artistName | 主播 | string | 4   | 否   | &nbsp; |
...
示例2.2.3：根据声音id批量查询播放链接 (注意：不要过于频繁请求，可能会触发限流)

    {
        "deviceId": "uid388808088185088",
        "request":
        {
            "id": "2635599741571190498",
            "launchApp": "",
            "action": "batchGetPlayUrl",
            "resultType":
            [
                "extendParam"
            ],
...
| |total | 总数 | int | 3   | &nbsp; |
| |albumId | 专辑Id，有声书类型返回 | string | 3   | &nbsp; |
|batchGetPlayUrl |trackId | 声音Id | string | 3   | &nbsp; |
| |playUrl | 音频播放地址，加密 | string | 3   | &nbsp; |
...
示例2.3.3：返回根据声音id批量查询播放链接结果

    {
        ...
        "result":
        {
            ...
            [
                {
                    "playUrl": "",
                    "trackId": "net_ease_1411304534"
                },
                {
                    "playUrl": "",
                    "trackId": "net_ease_1408447170"
                }
            ],
            "id": "2635599741571190498",
            "resultType":
